#!/usr/bin/env node
/**
 * Extract _hyperscript Patterns from Official Test Suite (v2 - Recursive)
 *
 * This script recursively scans the official _hyperscript test suite and extracts
 * all unique patterns found in _="" attributes and test definitions.
 */
...
async function extractPatterns() {
  ...
  try {
    const testFiles = await findTestFilesRecursively(OFFICIAL_TEST_DIR);
    ...
    for (const filePath of testFiles) {
      try {
        ...
        const relativeFilePath = filePath.replace(OFFICIAL_TEST_DIR, '');
        ...
        if (filesProcessed % 10 === 0) {
          process.stdout.write(`\r   Processed ${filesProcessed}/${testFiles.length} files...`);
        }
      } catch (error) {
    ...
    console.log(`\r   Processed ${filesProcessed}/${testFiles.length} files ✓`);
...
async function findTestFilesRecursively(dir) {
  const files = [];
