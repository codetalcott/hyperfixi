...
| `packages/{speech,playground,components,intercept}` + `experiments/{dixi,fixi-spike,grail-demo,siren}` + `protocol/{python,rust}` | `archived/peripheral-2026h1`           | Zero dependents across all sibling repos, 3–5 months cold (protocol spec + typescript + go parsers kept). `planner` was on the candidate list but is KEPT — live dep of mcp-server's GRAIL tools | 2026-08-07 |
