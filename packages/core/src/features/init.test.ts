...
describe.skip('Init Feature', () => {
  ...
  describe('Basic Initialization', () => {
    ...
    it('should have access to element as "me" in context', async () => {
      const commands = [
        { type: 'command', name: 'set', args: ['attribute', 'data-processed', 'true'] },
      ];
      initFeature.registerElement(testElement, commands, false);
      ...
      const commands = [
        {
          type: 'command',
          name: 'set',
          args: ['local', 'config', 'behaviorElement.dataset.config'],
        },
      ...
      const commands = [
        {
          type: 'command',
          name: 'set',
          args: ['local', 'removeButton', 'me.querySelector(".remove-btn")'],
        },
  ...
  describe('Error Handling', () => {
    ...
    it('should emit error events on initialization failure', async () => {
      ...
      const commands = [{ type: 'command', name: 'throw', args: ['Init error'] }];
      initFeature.registerElement(testElement, commands, false);
    ...
    it('should provide meaningful error messages with context', async () => {
      ...
      const errorCall = errorSpy.mock.calls.find(
        call => call[0].includes('init') && call[0].includes(testElement.id || 'element')
      );
  ...
  describe('Validation and Security', () => {
    ...
    it('should handle malformed element references', async () => {
      ...
      await expect(initFeature.processElement(null as any, context)).resolves.not.toThrow();

      // Test with undefined element
      await expect(initFeature.processElement(undefined as any, context)).resolves.not.toThrow();
    });
  });
});
