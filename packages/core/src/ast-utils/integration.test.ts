...
describe('Integration Tests - Real World AST Analysis', () => {
  ...
  it('should query for specific patterns in real code', () => {
    ...
    const commandsWithSelectors = findNodes(
      ast,
      node => node.type === 'command' && (node as any).target?.type === 'selector'
    );
          ...
          commands: [
            {
              ...
              name: 'toggle',
              args: [
                { type: 'selector', value: '.modal-visible', start: 0, end: 5, line: 1, column: 1 },
              ],
