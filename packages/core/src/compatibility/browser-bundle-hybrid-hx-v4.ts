...
async function executeOnElement(code: string, element: Element): Promise<void> {
  // `traditional: true` — this source is generated by the htmx translator, not
  // written by a human in some natural language, so the semantic (multilingual)
  // parser has nothing to contribute. It also actively loses information: it
  // drops `fetch`'s `with { ... }` options clause, which is where `hx-headers`
  // and `hx-vals` live. Forcing the traditional parser keeps them on the wire.
  await hyperfixiAPI.run(code, element, { traditional: true });
}
