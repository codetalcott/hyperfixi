...
describe('In Operator - TDD Fix', () => {
  describe('Array Membership Tests', () => {
    it('should handle item in array: 2 in items = true', async () => {
      const result = await evaluateExpressionFromSource('2 in items', context);
      expect(result).toBe(true);
    });

    it('should handle item not in array: 6 in items = false', async () => {
      const result = await evaluateExpressionFromSource('6 in items', context);
      expect(result).toBe(false);
    });

    it('should handle empty array: 1 in emptyArray = false', async () => {
      const result = await evaluateExpressionFromSource('1 in emptyArray', context);
      expect(result).toBe(false);
    });

    it('should handle string in array: "hello" in ["hello", "world"] = true', async () => {
      const result = await evaluateExpressionFromSource('"hello" in ["hello", "world"]', context);
      expect(result).toBe(true);
    });
  });
  ...
  describe('Object Property Tests', () => {
    it('should handle property in object: "name" in obj = true', async () => {
      const result = await evaluateExpressionFromSource('"name" in obj', context);
      expect(result).toBe(true);
    });

    it('should handle property not in object: "missing" in obj = false', async () => {
      const result = await evaluateExpressionFromSource('"missing" in obj', context);
      expect(result).toBe(false);
    });
    ...
    it('should handle empty object: "prop" in emptyObj = false', async () => {
      const result = await evaluateExpressionFromSource('"prop" in emptyObj', context);
      expect(result).toBe(false);
    });
  });
  ...
  describe('Operator Precedence', () => {
    it('should have correct precedence: 1 + 2 in [3, 4] = true', async () => {
      ...
      const result = await evaluateExpressionFromSource('1 + 2 in [3, 4]', context);
      expect(result).toBe(true);
    });

    it('should work with parentheses: (2 in items) and true = true', async () => {
      const result = await evaluateExpressionFromSource('(2 in items) and true', context);
      expect(result).toBe(true);
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle null values: null in [null, 1, 2] = true', async () => {
      const result = await evaluateExpressionFromSource('null in [null, 1, 2]', context);
      expect(result).toBe(true);
    });

    it('should handle boolean values: true in [true, false] = true', async () => {
      const result = await evaluateExpressionFromSource('true in [true, false]', context);
      expect(result).toBe(true);
    });
  });
