...
const spanishKeywords: KeywordResolver = {
  ...
  isCommand(token: string): boolean {
    const commands = new Set([
      'en', 'clic', 'alternar', 'poner', 'establecer', 'si', 'registrar',
      'esperar', 'mostrar', 'ocultar', 'agregar', 'quitar',
      // English fallbacks
      'on', 'click', 'toggle', 'put', 'set', 'if', 'log', 'wait', 'show', 'hide', 'add', 'remove',
    ]);
    return commands.has(token.toLowerCase());
  },
  isKeyword(token: string): boolean {
    const keywords = new Set([
      'entonces', 'sino', 'y', 'o', 'no', 'a', 'de', 'con',
      'then', 'else', 'and', 'or', 'not', 'to', 'from', 'with',
    ]);
...
const japaneseKeywords: KeywordResolver = {
  ...
  isCommand(token: string): boolean {
    const commands = new Set([
      'で', 'クリック', '切り替え', '置く', '設定', 'もし', '記録', '待つ', '表示', '隠す',
      // English fallbacks
      'on', 'click', 'toggle', 'put', 'set', 'if', 'log', 'wait', 'show', 'hide',
    ]);
    return commands.has(token.toLowerCase());
  },
  isKeyword(token: string): boolean {
    const keywords = new Set([
      'それから', 'そうでなければ', 'そして', 'または', 'ではない', 'に', 'から', 'と',
      'then', 'else', 'and', 'or', 'not', 'to', 'from', 'with',
    ]);
    return keywords.has(token.toLowerCase());
  },
...
function isEnglishKeyword(token: string): boolean {
  const englishKeywords = new Set([
    'on', 'click', 'toggle', 'put', 'set', 'if', 'log', 'wait', 'show', 'hide',
    'add', 'remove', 'then', 'else', 'and', 'or', 'not', 'to', 'from', 'with',
    'true', 'false', 'me', 'it', 'my', 'seconds', 'ms',
  ]);
