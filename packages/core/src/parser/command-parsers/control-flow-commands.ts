...
export function parseIfCommand(ctx: ParserContext, commandToken: Token): CommandNode {
  ...
  // accepts — see docs-internal/archive/handoffs/HANDOFF-command-word-in-if-condition.md.
  let hasThen = false;
  ...
  if (!hasThen) {
    ...
    while (!ctx.isAtEnd() && ctx.current - savedPosition < maxLookahead) {
      ...
      // it. See docs-internal/archive/handoffs/HANDOFF-implicit-multiline-if.md.
  ...
  if (isMultiLine) {
    ...
    // docs-internal/archive/handoffs/HANDOFF-command-word-in-if-condition.md). The command guard
