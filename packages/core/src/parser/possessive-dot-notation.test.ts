...
describe('Possessive dot notation (my.prop, its.prop, your.prop)', () => {
  describe('my.property syntax', () => {
    ...
    it('should parse my.textContent (dot syntax)', async () => {
      const result = await parseAndEvaluateExpression('my.textContent', context as unknown as ExecutionContext);
      expect(result).toBe('hello world');
    });

    it('should parse my.value (dot syntax)', async () => {
      const result = await parseAndEvaluateExpression('my.value', context as unknown as ExecutionContext);
      expect(result).toBe('test value');
    });

    it('should parse my.className (dot syntax)', async () => {
      const result = await parseAndEvaluateExpression('my.className', context as unknown as ExecutionContext);
      expect(result).toBe('btn primary');
    });

    it('should handle chained access: my.parentElement.id', async () => {
      const result = await parseAndEvaluateExpression('my.parentElement.id', context as unknown as ExecutionContext);
    ...
    it('should be equivalent to me.textContent', async () => {
      const myResult = await parseAndEvaluateExpression('my.textContent', context as unknown as ExecutionContext);
      const meResult = await parseAndEvaluateExpression('me.textContent', context as unknown as ExecutionContext);
  ...
  describe('its.property syntax', () => {
    ...
    it('should parse its.value (dot syntax)', async () => {
      const result = await parseAndEvaluateExpression('its.value', context as unknown as ExecutionContext);
      expect(result).toBe('it-value');
    });

    it('should parse its.name (dot syntax)', async () => {
      const result = await parseAndEvaluateExpression('its.name', context as unknown as ExecutionContext);
      expect(result).toBe('it-name');
    });

    it('should be equivalent to it.value', async () => {
      const itsResult = await parseAndEvaluateExpression('its.value', context as unknown as ExecutionContext);
      const itResult = await parseAndEvaluateExpression('it.value', context as unknown as ExecutionContext);
  ...
  describe('your.property syntax', () => {
    ...
    it('should parse your.value (dot syntax)', async () => {
      const result = await parseAndEvaluateExpression('your.value', context as unknown as ExecutionContext);
      expect(result).toBe('you-value');
    });

    it('should parse your.name (dot syntax)', async () => {
      const result = await parseAndEvaluateExpression('your.name', context as unknown as ExecutionContext);
      expect(result).toBe('you-name');
    });

    it('should be equivalent to you.name', async () => {
      const yourResult = await parseAndEvaluateExpression('your.name', context as unknown as ExecutionContext);
      const youResult = await parseAndEvaluateExpression('you.name', context as unknown as ExecutionContext);
  ...
  describe('method calls with possessive dot notation', () => {
    ...
    it('should support method calls: my.getAttribute("data-value")', async () => {
      const result = await parseAndEvaluateExpression('my.getAttribute("data-value")', context as unknown as ExecutionContext);
