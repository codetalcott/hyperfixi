...
describe('If Command', () => {
  ...
  describe('Command Properties', () => {
    it('should have correct metadata', () => {
      ...
      expect(command.syntax).toBe('if <conditional> [then] <command-list> [(else | otherwise) <command-list>] end`');
      expect(command.description).toBe('The if command provides the standard if-statement control flow.');
    });
  });
