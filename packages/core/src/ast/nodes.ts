 ...
 * per-kind set in `types/base-types.ts` (its eleven per-kind interfaces deleted
 * by step 4: seven had no importer at all, the live four now resolve here;
 * `parser/parser-types.ts`' fifteen remain, for the measured reason in
