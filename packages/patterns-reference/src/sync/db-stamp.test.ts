...
describe('db-stamp provenance guard', () => {
  ...
  it('folds the renderer choice in: a PATTERNS_RENDERER=best DB is stale to a default gate run', () => {
    const { root, dbPath } = makeFakeRepo();
    roots.push(root);
    ...
    try {
      ...
      writeDbStamp(dbPath); // what a `PATTERNS_RENDERER=best npm run populate` leaves behind
      expect(computeDbInputHash(dbPath)).not.toBe(h1);
