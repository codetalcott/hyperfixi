...
describe('CLI', () => {
  ...
  describe('version reporting', () => {
    // Equality, not shape: the literal this replaced was '0.1.0' against a
    // published 2.10.0, and a `/^\d+\.\d+\.\d+/` assertion would have passed on
    // it forever. `src/version.ts` is generated by scripts/set-version.cjs, so
    // this fails if a release bump ever stops rewriting it.
    it('reports the real package version', async () => {
      const { VERSION } = await import('./version');
      const pkgPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../package.json');
      const declared = JSON.parse(readFileSync(pkgPath, 'utf8')).version as string;
