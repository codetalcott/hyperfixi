...
const QUECHUA_EXTRAS: KeywordEntry[] = [
  ...
  // Glottalized spelling — the curated corpus rows (the since-deleted
  // fix-translations.sql) and the event-handler pattern map both use ñit'iy.
  // Without this entry the word-walk splits it at the injected
  // English-passthrough keyword `it` (ñ + it + 'iy), wrecking every curated qu
  // click pattern.
