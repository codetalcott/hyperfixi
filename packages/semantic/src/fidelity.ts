/**
 * Structural fidelity for multilingual parses — importable as
 * `@lokascript/semantic/fidelity`.
 *
 * Extracted verbatim from `@hyperfixi/testing-framework` (agent-era arc 4),
 * which now re-exports from here: these are pure functions over parsed node
 * trees with zero dependencies — no corpus, no baseline, no DOM — so any
 * consumer (the CI ratchets, the compilation service's scoring API, an
 * external harness verifying LLM-generated code) can score a candidate parse
 * against a reference without pulling in the test framework's machinery.
 * docs/FIDELITY.md documents the methodology and its measured blind spots.
 *
