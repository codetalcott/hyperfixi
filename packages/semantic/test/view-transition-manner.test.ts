 ...
 *
 * The split is MEASURED. A language moving from DEFERRED to CAPTURED is
 * progress: move it up in the same change. Moving one the other way is a
 * regression.
 *
 * These use semantic's own `render()`. The surfaces the multilingual GATE
...
/**
 * The STORED corpus surfaces — what the multilingual gate actually scores.
 *
 * `patterns.db` is generated by the i18n `GrammarTransformer`, not by semantic's
 * `render()`, and the two disagree: `render()` emits the schema's canonical
 * pattern shape, the transformer emits verb-MEDIAL event handlers
 * (`#a を クリック で 交換 #b で`) that no generated command pattern covers.
 * So the round-trips above neither cover nor constrain these rows.
 *
 * With the corpus row `swap-view-transition` added, this is measured ground:
 * before the SOV/VSO event-handler slot existed, all eight of ar/bn/hi/ja/ko/
 * qu/tl/tr matched `swap` at confidence 1.0 and reported
 * `fused body walk left 3 token(s) unconsumed: "using view transition"` — an R1
 * role loss that every action-based recall signal (R0, precision, multiset, R5)
 * is structurally incapable of seeing.
 *
 * Surfaces are frozen fixtures, not computed: importing @lokascript/i18n here
 * would invert the dependency direction. Regenerate with
 * `new GrammarTransformer('en', lang).transform(EN)` if the transformer's
 * emission shape changes — a diff here is a corpus rewrite, which needs a
 * baseline regen either way.
 */
describe('the stored corpus surfaces capture the tail in all 24 languages', () => {
  const EN = 'on click swap #a with #b using view transition';

  const STORED: Array<[string, string]> = [
    ['en', EN],
    ['es', 'en clic intercambiar #a con #b using view transition'],
    ['pt', 'em clique trocar #a com #b using view transition'],
    ['fr', 'sur clic échanger #a avec #b using view transition'],
    ['de', 'bei klick tauschen #a mit #b using view transition'],
    ['it', 'su clic scambiare #a con #b using view transition'],
    ['ja', '#a を クリック で 交換 #b で using view transition'],
    ['ko', '#a 를 클릭 할 때 교환 #b 로 using view transition'],
    ['zh', '当 点击 时 交换 把 #a 用 #b using view transition'],
    ['ar', 'استبدل #a عند نقر بـ#b using view transition'],
    ['he', 'ב לחיצה החלף את #a עם #b using view transition'],
    ['hi', '#a को क्लिक पर विनिमय #b से using view transition'],
    ['bn', '#a কে ক্লিক এ বদল #b দিয়ে using view transition'],
    ['tr', '#a i tıklama de takas #b ile using view transition'],
    ['ru', 'при клик поменять #a с #b using view transition'],
    ['uk', 'при клік поміняти #a з #b using view transition'],
    ['pl', 'gdy kliknięcie zamień #a z #b using view transition'],
    ['id', 'pada klik tukar #a dengan #b using view transition'],
    ['vi', 'khi nhấp hoán đổi #a với #b using view transition'],
    ['th', 'เมื่อ คลิก สลับที่ #a ด้วย #b using view transition'],
    ['ms', 'apabila click tukar_tempat #a dengan #b using view transition'],
    ['tl', 'palitan_pwesto #a kapag click nang #b using view transition'],
    ['sw', 'kwenye bonyeza badilishana #a na #b using view transition'],
    ['qu', "#a ta ñitiy pi t'inkuy #b wan using view transition"],
  ];

  it.each(STORED)('[%s] captures all three roles with nothing unconsumed', (lang, surface) => {
    const node = commandNode(surface, lang, 'swap');
    expect(roleValue(node, 'destination'), `${lang} destination`).toBe('#a');
    expect(roleValue(node, 'patient'), `${lang} patient`).toBe('#b');
    expect(roleValue(node, 'manner'), `${lang} manner`).toBe('transition');
    expect(unconsumedSpans(surface, lang), `${lang} unconsumed`).toEqual([]);
  });

  it.each(STORED)('[%s] reports the same role signature as the en reference (R1)', (lang, surface) => {
    // R1 compares `action.role:valueType`, and `transition` tokenizes as a
    // KEYWORD in en/fr (it is also their verb) and as a bare identifier in the
    // other 22 — `literal` vs `expression` for the identical word. The slot's
    // `valueShape: 'keyword'` normalizes it, so a divergence here would put
    // this row into 22 languages' `roleLossyPatterns` while every recall signal
    // stayed green.
    const sig = (src: string, l: string): string[] =>
      walkCommands(parse(src, l))
        .flatMap(c => [...c.roles.entries()].map(([r, v]) => `${c.action}.${r}:${(v as any).type}`))
        .sort();
