...
describe('Japanese fetch keyword alignment (フェッチ)', () => {
  ...
  // docs-internal/archive/plans/MULTILINGUAL_ROADMAP.md ("fetch keyword alignment — ja").
...
describe('Custom (non-keyword) event identifiers in SOV languages', () => {
  ...
  // Korean). See docs-internal/archive/plans/MULTILINGUAL_ROADMAP.md (on-custom-event-receive).
  const cases: Array<[string, string]> = [
...
describe('Trailing event clause wraps a block body (unless-condition, ar+tl)', () => {
  ...
  // See docs-internal/archive/plans/MULTILINGUAL_ROADMAP.md (unless-condition).
  const cases: Array<[string, string]> = [
...
describe('vi render keyword alignment (kết xuất, not the show-colliding hiển thị)', () => {
  ...
  // to `kết xuất`. See docs-internal/archive/handoffs/HANDOFF-lossy-tail.md (render cluster).
  const cases: Array<[string, string]> = [
...
describe('qu append keyword alignment (qatichiy, not the _-splitting qhipaman_yapay)', () => {
  ...
  // `qatichiy`. See docs-internal/archive/handoffs/HANDOFF-lossy-tail.md (singleton tail).
...
describe('zh tell BA-marked target (告诉 把 #el — tellSchema markerOverride zh:把)', () => {
  ...
  // tellSchema's destination markerOverride. See docs-internal/archive/handoffs/HANDOFF-lossy-tail.md.
  const cases: Array<[string, string[]]> = [
...
describe('ko if-empty: command verb directly after copula in a split SOV predicate', () => {
  ...
  // See docs-internal/archive/handoffs/HANDOFF-lossy-tail.md (control-flow if-folding).
...
describe('hi hyphen-compound keyword (साफ़-करें = clear) tokenizes as one keyword', () => {
  ...
  // run when it resolves to a registered keyword. See docs-internal/archive/handoffs/HANDOFF-lossy-tail.md.
...
describe('Multi-token event names anchor the event handler (ar multi-word, sw underscore)', () => {
  ...
  // identifier reader keeps `_` inside a word. See docs-internal/archive/handoffs/HANDOFF-lossy-tail.md.
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.add(rec.action);
...
describe('qu repeat loop-keyword swallow guard (verb-final wait eats the loop kw)', () => {
  ...
  // emits the bare repeat instead. See docs-internal/archive/handoffs/HANDOFF-lossy-tail.md (Arc 2).
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.add(rec.action);
...
describe('ar measure keyword alignment (undiacritized قس)', () => {
  ...
  // See docs-internal/archive/handoffs/HANDOFF-lossy-tail.md (Arc 2 — ar measure).
...
describe('Caret-scoped variable read `^name on <element>` (caret-var-on-target)', () => {
  ...
  // See docs-internal/archive/plans/MULTILINGUAL_ROADMAP.md (caret-var-on-target).
...
describe('German fetch keyword alignment (abrufen vs holen)', () => {
  ...
  // See docs-internal/archive/plans/MULTILINGUAL_ROADMAP.md ("German fetch keyword alignment").
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
...
describe('if/else block-body in event handlers — Track 5 Tier 1', () => {
  ...
  // degenerate to faithful. See docs-internal/archive/plans/MULTILINGUAL_ROADMAP.md (Track 5 Tier 1).
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const n = node as Record<string, unknown>;
    if (typeof n.action === 'string') acc.add(n.action);
...
describe('fused-event trailing `if … end` folds + verb-medial set (fetch-do-not-throw, SOV)', () => {
  ...
  // docs-internal/archive/handoffs/HANDOFF-fetch-do-not-throw.md.
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const n = node as Record<string, unknown>;
    if (typeof n.action === 'string') acc.add(n.action);
...
describe('async modifier transparency — Track 5 Async Tier 1', () => {
  ...
  // See docs-internal/archive/plans/MULTILINGUAL_ROADMAP.md (Track 5 Async Tier 1).
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const n = node as Record<string, unknown>;
    if (typeof n.action === 'string') acc.add(n.action);
...
describe('then/end keyword recognition for profile-only languages — Track 5', () => {
  ...
  // byte-identical). See docs-internal/archive/plans/MULTILINGUAL_ROADMAP.md (then/end recognition).
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const n = node as Record<string, unknown>;
    if (typeof n.action === 'string') acc.add(n.action);
...
describe('Juxtaposed multi-command event bodies — Track 5', () => {
  ...
  // + others degenerate→faithful. See docs-internal/archive/plans/MULTILINGUAL_ROADMAP.md.
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const n = node as Record<string, unknown>;
    if (typeof n.action === 'string') acc.add(n.action);
...
describe('SOV verb-first event-body reorder — modifier-prefixed bodies (Track 5)', () => {
  ...
  // the parser must recover every body command. See docs-internal/archive/scopes/SOV_REORDER_SCOPE.md.
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.add(rec.action);
...
describe('SOV put-into verb-final reorder — ko/tr/bn (Track 5)', () => {
  ...
  // docs-internal/archive/plans/MULTILINGUAL_ROADMAP.md (SOV put-into reorder).
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.add(rec.action);
...
describe('SOV repeat-* loop-body reorder — ko/bn/qu (Track 5)', () => {
  ...
  // docs-internal/archive/scopes/SOV_REPEAT_SCOPE.md. Strings below are post-transform output
...
describe('VSO/Austronesian repeat-* mid-stream event reorder — ar/tl (Track 5)', () => {
  ...
  // post-transform output (en → lang). See docs-internal/archive/scopes/NON_SOV_REPEAT_SCOPE.md.
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.add(rec.action);
...
describe('Non-SOV repeat-* loop-body + tail residue — zh/ar/tl/ja/ko/sw (Track 5)', () => {
  ...
  // docs-internal/archive/scopes/NON_SOV_REPEAT_SCOPE.md. Strings below are post-transform output
...
describe('qu/sw increment keyword alignment (yapachiy / ongezeko)', () => {
  ...
  // follow-up to docs-internal/archive/scopes/NON_SOV_REPEAT_SCOPE.md.
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.add(rec.action);
...
describe('zh wait BA-marked duration (等待 把 {duration})', () => {
  ...
  // mark a duration as a fronted patient) is scoped in docs-internal/archive/scopes/ZH_BLOCK_BODY_SCOPE.md.
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.add(rec.action);
...
describe('zh then-connective 那么 recognized (aligns with i18n)', () => {
  ...
  // recovery no longer leans on the fallback. See docs-internal/archive/scopes/ZH_BLOCK_BODY_SCOPE.md.
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.add(rec.action);
...
describe('zh fetch in event block (抓取 把 {source} [的 {responseType}])', () => {
  ...
  // verb 放置 + separate 到 marker). See docs-internal/archive/scopes/ZH_BLOCK_BODY_SCOPE.md (#3).
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.add(rec.action);
...
describe('focus command keyword alignment (de/fr/pl/pt/sw)', () => {
  ...
  // See docs-internal/archive/plans/MULTILINGUAL_ROADMAP.md ("focus keyword alignment").
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.add(rec.action);
...
describe('socket command keyword alignment (9 native-primary languages)', () => {
  ...
  // docs-internal/archive/plans/MULTILINGUAL_ROADMAP.md ("socket keyword alignment") and
  // docs-internal/archive/scopes/BLOCK_BODY_CONDITION_SCOPE.md (Phase 0).
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.add(rec.action);
...
describe('eventsource / worker profile entries (hi, tl) — Phase 0b', () => {
  ...
  // docs-internal/archive/scopes/BLOCK_BODY_CONDITION_SCOPE.md (Phase 0b).
  function actions(node: unknown, acc = new Set<string>()): Set<string> {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.add(rec.action);
...
// per-language mechanism; see docs-internal/archive/plans/STRUCTURAL_ARCS_ROADMAP.md.
// =============================================================================
...
describe('Turkish unless keyword alignment (değilse)', () => {
  ...
  // Those remain lossy — see docs-internal/archive/handoffs/HANDOFF-unless-condition-tokenizer.md.
  //
...
describe('Trailing SOV `unless` guard recovery (unless-condition, ko/bn/ja)', () => {
  ...
  // See docs-internal/archive/handoffs/HANDOFF-unless-condition-tokenizer.md.
  const collectActions = (node: unknown, acc: string[] = []): string[] => {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.push(rec.action);
...
describe('Bare-event mis-anchor guard (hi unless-condition — SOV event-anchor #5)', () => {
  ...
  // docs-internal/archive/handoffs/HANDOFF-unless-condition-tokenizer.md.
...
describe('`do not throw` fetch modifier strip (fetch-do-not-throw phantom-throw)', () => {
  ...
  // handoff (docs-internal/archive/handoffs/HANDOFF-fetch-do-not-throw.md).
  const collectActions = (node: unknown, acc: string[] = []): string[] => {
    if (!node || typeof node !== 'object') return acc;
    const rec = node as Record<string, unknown>;
    if (typeof rec.action === 'string' && rec.action !== 'compound') acc.push(rec.action);
...
describe('colon-qualified event names: full behavior bodies capture every trigger by VALUE (hi/ja/ko/ms/qu)', () => {
  ...
  // hi/ja/ko/ms/qu (docs-internal/archive/handoffs/HANDOFF_colon-event-names.md). Three stacked
...
describe('R3 value-bug families (docs-internal/archive/handoffs/HANDOFF_value-bug-families.md)', () => {
...
describe('R1 Family A: trailing SOV with-options blob reclaimed as fetch/render style (docs-internal/archive/handoffs/HANDOFF_r1-role-fidelity.md)', () => {
...
describe('R1 Family B: qu set — oblique manta source + whole backtick templates (docs-internal/archive/handoffs/HANDOFF_r1-role-fidelity.md)', () => {
...
describe('R1 Family C: tr/qu verb-final or-run wait captures the first event (docs-internal/archive/handoffs/HANDOFF_r1-role-fidelity.md)', () => {
...
describe('R1 Family D: SOV fallback value-typing increments (docs-internal/archive/handoffs/HANDOFF_r1-role-fidelity.md)', () => {
...
describe('R1 deferred-tail Family E: URL extractor carries ${…} spans (docs-internal/archive/handoffs/HANDOFF_r1-deferred-tail.md)', () => {
...
describe('R1 deferred-tail Family H: fused halt+call renders verb-final and parses like en (docs-internal/archive/handoffs/HANDOFF_r1-deferred-tail.md)', () => {
...
describe('R1 deferred-tail Family G: SOV focus-trap branch operand survives the fold (docs-internal/archive/handoffs/HANDOFF_r1-deferred-tail.md)', () => {
...
describe('R1 deferred-tail qu tail: per-row alignments (docs-internal/archive/handoffs/HANDOFF_r1-deferred-tail.md)', () => {
...
describe('Spanish hacia destination alignment (vocab Batch 1, V2+V4)', () => {
  ...
  // docs-internal/archive/handoffs/HANDOFF_vocab-batch1-v4-probe.md → MULTILINGUAL_NEXT_STEPS.md
  // § "V4 probe conclusion".
...
describe('Foreign-validity Phase 11: bn অথবা→or + verb-first or-run wait (docs-internal/archive/handoffs/HANDOFF_foreign-validity-burndown.md)', () => {
