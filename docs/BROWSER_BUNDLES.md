...
4. **Pure hyperscript (`_=` attributes), ~85% feature coverage, smallest realistic size?** Use `hyperfixi-hybrid-complete.js` (~8.1 KB gz). Full AST parser, expressions, event modifiers, block commands (`if`, `for`, `repeat`, `while`, `fetch`).
...
| `hyperfixi-hybrid-complete.js` | 8.1 KB      | 21+blocks | Full AST parser, expressions, event modifiers                 |
