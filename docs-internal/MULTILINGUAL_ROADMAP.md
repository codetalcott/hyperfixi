...
_Last updated 2026-07-05: **this roadmap's mission is COMPLETE — parse rate 3696/3696 (100%), degenerate 0, lossy 0, avgFidelity 1.000, all holding under the six-signal ratchet gate.** The lone `tr window-resize` hard-fail noted below has since been cleared, closing the last parse-rate item. The active multilingual work has moved UP the fidelity stack and is tracked in [MULTILINGUAL_NEXT_STEPS.md](MULTILINGUAL_NEXT_STEPS.md) (the "read this first" entry point): the R1 role-fidelity burn-down took avgRoleFidelity **0.845 → 0.977** across the SOV event-anchor arc, the five-cluster residual triage (#564–#568), and two residue sweeps (#569/#571/#572, #573) — per-arc grounding in [HANDOFF-r1-residual.md](archive/HANDOFF-r1-residual.md) and [HANDOFF-r1-post-cluster-residue.md](archive/HANDOFF-r1-post-cluster-residue.md). Remaining headroom: the SOV six's R1 tail (~0.95) and the R0-precision spurious-action families. This document stays as the historical record of the parse-rate and lossy/degenerate-band campaigns; new session logs go to NEXT_STEPS._

_Earlier: **the lossy band is CLEARED (lossy → 0) — every parsing pattern is now faithful.** Across #495–#506 the remaining lossy tail (53 → 0, after the degenerate band hit 0 in #492/#493) was swept, almost entirely via localized-alignment fixes (a dict keyword, tokenizer split, or per-language role marker): th `trigger`/`send` (#495), per-language `of`-possessive markers (#497), `get-value` de/pl/zh (#498), `form-submit-prevent` he/zh (#499), `unless-condition` qu/vi/zh (#501), vi `render` (#502), qu `append` (#503), zh `tell` (#504), and the ar/qu/sw loop-body + ko if-fold residue (#506). Authoritative baseline (commit `12018416`, `browser-priority`): parse rate **3695/3696** (lone hard-fail `tr window-resize`), degenerate **0**, lossy **0**, faithful **3695**, avgFidelity **1.000**, avgPrecision **0.971**, avgRoleFidelity **0.845** (the remaining laggard — hi 0.757 · bn 0.784 · qu 0.785). The only remaining multilingual fidelity work is the **R1/SOV role-fidelity burn-down** (Track 3 / the convergent SOV event-anchor arc) and the deferred `tr window-resize` hard-fail; per-fix grounding + the remaining arcs are in [MULTILINGUAL_NEXT_STEPS.md](MULTILINGUAL_NEXT_STEPS.md) + [HANDOFF-lossy-tail.md](archive/HANDOFF-lossy-tail.md)._

_Earlier: Track 5 **marker-less `fetch` fidelity sweep (es/pl/id/sw/he)** — generalizes the fr/pt fix (#329). A scoping probe of the "`${...}`-in-URL" thread found that interpolation is a red herring (it works fine in backtick template literals everywhere); the real, broader issue is the **marker-less bare-`fetch` gap**: for `fetch <url>` (no `from`) the transformer emits a marker-less `buscar /api/data` / `pobierz /api/data` / …, but the generated pattern requires a source marker, so `fetch` **dropped** across ~5 fetch patterns (fetch-basic, async-block, fetch-json, fetch-with-method, fetch-loading-state) in es/pl/id/sw/he. These sat at ~0.67 fidelity — **faithful-but-incomplete** passes (above the 0.50 floor, so invisible to the degenerate metric) that silently lost the `fetch` command. Handcrafted patterns (optional source marker + responseType, mirroring `fetch-fr`/`fetch-ms`) recover `fetch`; id additionally accepts the dict verb `ambil` (profile primary `muat`), he the `את` accusative particle (`הבא את /url`) the transformer inserts where the pattern expects `מ`. **avgFidelity ↑ es 0.917→0.951, he 0.876→0.910, id 0.861→0.895, pl 0.901→0.935 (all +3.4pt), sw 0.924→0.929**; **sw `event-debounce` also flips degenerate → faithful (degenerate total 70 → 69, −1)**, `--regression` gate green, **0 regressions** (0 parse-rate drops, 0 degenerate additions, no avgConfidence churn); baseline regenerated. Locked by `multilingual-roadmap-fixes.test.ts` ("marker-less fetch fidelity"). The remaining bare-fetch droppers (de `holen`→`get` keyword; ja/zh SOV/topic marker forms) are distinct and left for a follow-up._
...
> avgRoleFidelity **0.845** (laggard — hi 0.757 · bn 0.784 · qu 0.785). **Both correctness bands
> (degenerate + lossy) are empty: every parsing pattern is faithful.** The remaining work is the
> R1/SOV role-fidelity dimension (Track 3) + the lone `tr window-resize` hard-fail — see
> [MULTILINGUAL_NEXT_STEPS.md](MULTILINGUAL_NEXT_STEPS.md). The 2026-06-17 snapshot below is the
> prior state; authoritative numbers always live in the baseline JSON.
...
> [CORRECTNESS_RELIABILITY_PLAN.md](archive/CORRECTNESS_RELIABILITY_PLAN.md).
...
- **Remaining B1 (deferred, genuinely hard):** he (`unless`/`if-empty` — English
  ...
  handler to a bare `blur`). See [BLOCK_BODY_CONDITION_SCOPE.md](archive/BLOCK_BODY_CONDITION_SCOPE.md) §3.3.
...
- **Deferred within B1:** **he** (transformer leaves `value is empty` in English) and
  **ja/ko** (SOV reorder splits `is`…`empty`) — the next 1b/1c increments. See
  [BLOCK_BODY_CONDITION_SCOPE.md](archive/BLOCK_BODY_CONDITION_SCOPE.md) §3.
...
- **Deferred:** `install-behavior` (hi/ru/uk, 3 instances) — a lexical `set`/`install`
  ...
  [BLOCK_BODY_CONDITION_SCOPE.md](archive/BLOCK_BODY_CONDITION_SCOPE.md) §2c.
...
- **Context.** This is **Phase 0** of the block-body decomposition scoped in
  [BLOCK_BODY_CONDITION_SCOPE.md](archive/BLOCK_BODY_CONDITION_SCOPE.md): the "block-body
...
- **Deeper follow-up (deferred).** The transformer over-applies `把` to non-patient
  ...
  [ZH_BLOCK_BODY_SCOPE.md](archive/ZH_BLOCK_BODY_SCOPE.md). Locked by
...
- **Deferred (separate keyword-alignment tracks):** the zh `wait` SVO pattern gap
  ...
  [NON_SOV_REPEAT_SCOPE.md](archive/NON_SOV_REPEAT_SCOPE.md).
...
- **Honest scope.** ar/tl `repeat-for-each` land at 0.67 (the `for <var> in <coll>`
  ...
  tail residue across SOV. All scoped in [NON_SOV_REPEAT_SCOPE.md](archive/NON_SOV_REPEAT_SCOPE.md).
...
- Locked by `multilingual-roadmap-fixes.test.ts` ("SOV repeat-\* loop-body reorder":
  ...
  [SOV_REPEAT_SCOPE.md](archive/SOV_REPEAT_SCOPE.md).
...
- See [SOV_REORDER_SCOPE.md](archive/SOV_REORDER_SCOPE.md). Locked by
...
> campaign log, [CORRECTNESS_RELIABILITY_PLAN.md](archive/CORRECTNESS_RELIABILITY_PLAN.md).
...
- **`async-block` (13) is a separate root** — `async` is a command _modifier_ the
  ...
  [SOV_REORDER_SCOPE.md](archive/SOV_REORDER_SCOPE.md).**
...
5. **SOV `repeat-*` loop bodies** (ko/bn/qu, ~6) — block loops
   ...
   [SOV_REPEAT_SCOPE.md](archive/SOV_REPEAT_SCOPE.md).** Includes a separable quick win: qu's
