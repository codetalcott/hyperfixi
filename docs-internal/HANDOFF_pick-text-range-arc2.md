...
| range-`start` | absent as vocabulary (only `verb.position: 'start'` config) | **DEFERRED to arc 3** (dual collisions — see below) | colon-events (`draggable:start`) PROBED safe (dict masking + start-prefixed vars like `startX` use whole-token lookup — byte-identical). But 6 native start words ALREADY map to a keyword: hi `प्रारंभ`/bn `শুরু`/id `mulai`/ms `mula`/sw `anza` → `init`, qu `qallariy` → `default`; ar `بداية`/he `התחלה` split on their prefix; vi `bắt đầu` is multi-token. Safe-to-register set is only 14/23 (es/fr/pt/it/de/pl/ru/uk/ja/zh/ko/th/tr/tl). |
| range-`end` | **registered as block-end in EVERY language** | **DEFERRED to arc 3 with `start` (it's one mechanism)** | the bn `শেষ` dual class. When arc 3 lands the endpoints: `end` needs NO re-registration (block-end already normalizes; `tryConsumePickRangeOperand` :1332 accepts keyword `end` mid-fold; the connective guard `pattern-matcher.ts:~572-588` only rejects `end` as a role's FIRST token). en `start to end` endpoints ARE accepted by the arc-1 assembler — PROBED green 2026-07-20 (round-trips byte-identical, rangeStart/rangeEnd literals; see pick-command.test.ts). |
