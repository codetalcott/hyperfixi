...
| 4 ✅  | ~~`ASTCache` → Program cache~~ FALSIFIED on the design — Ops are per runtime, so the Program cache is the runtime's compile memo; the AST cache's identity guarantee is what makes it hit. Eviction test added. Was: the value gains `program`, the four-part key stays, and an eviction test is added (the current suite never crosses 500).                                                                                                  | `ast-cache.test.ts` exercises eviction                                                  | S    |
