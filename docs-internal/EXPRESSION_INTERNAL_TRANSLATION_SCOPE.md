...
| Family | pairs | kind |
| --- | --- | --- |
| **`document` + `window` via `profile.references`** | **17** | **data — the biggest actionable family; START HERE** |
...
§ "What is left". It was produced by rendering all 94 pairs and clustering by the canonical
...
- **"Context globals have zero reverse coverage"** — false, but the correction was ALSO
  wrong and stood for four phases. `body` is indeed **24/24** via `profile.references`.
  But the follow-on claim — "`window` is 1 (qu); only `document` is truly 0; worth ~1 pair
  standalone, the LAST slice by value, not a cheap first win" — is **false** (measured
  2026-07-17, post-#719). `document` leaks in **14** pairs and `window` in **3**: **17
  together, the biggest actionable family left, and pure `references` data.** The "~1"
  came from counting the qu `k_iri` tokenizer EXTRA and never rendering a single row. The
  leak is one line inside a 20-line behavior body (`repeat until event pointerup from
  ドキュメント`), which is why prose inspection kept missing it. **Two successive sessions
  restated this figure without probing it** — including the one that wrote "probe the claim
