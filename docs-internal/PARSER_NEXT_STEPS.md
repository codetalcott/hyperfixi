...
| **`js(...) … end` bodies are not lexed as an OPAQUE span** (semantic/framework tokenizer — `packages/framework/src/core/tokenization/base-tokenizer.ts`, not core's) | medium — the English semantic parse of `behavior-removable` leaves ~15 tokens of JavaScript unconsumed, every foreign render then TRANSLATES the JavaScript, and the engine rejects 16 of 23 languages' rows (`Unexpected token 'return'`) | ✅ `render-fidelity` + `bare-render-fidelity` allowlists (`behavior-removable` ×5 / ja), and — once the `best` corpus writer is the default — the i18n-kept row ratchet (19 of its 229 rows: `behavior-removable` ×16 + `js-inline` ×3) | Filed 2026-08-27; three handoffs pointed here without writing it (`~/.claude/plans/kickoff-en-foreign-render-residual.md` §3, §"Residual"; `MULTILINGUAL_NEXT_STEPS.md` 2026-08-27c). Not a multilingual bug: fix = lex `js(` … matching `end` as one opaque token in the base tokenizer, then the renderer passes the body through verbatim. |
