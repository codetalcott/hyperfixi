...
import type { SirenAction, SirenEntity, ConditionMapEntry } from './types';
...
export function inferConditions(
  ...
  for (const name of conditionNames) {
    ...
    if (m.op === 'eq' && String(val) === String(m.value)) conds.add(name);
      ...
      try { if (Number(val) > Number(m.value)) conds.add(name); } catch { /* skip */ }
    }
  }
 ...
 */
export function parseConditionMap(
  conditionMap: Record<string, ConditionMapEntry>,
): { defs: AffordanceDef[]; conditionNames: Set<string>; mutexPrefixes: Set<string> } {
 ...
 */
export function buildDefsFromActions(
  actions: SirenAction[],
): { defs: AffordanceDef[]; conditionNames: Set<string>; mutexPrefixes: Set<string> } {
...
export function planFromEntity(
  ...
  if (unmet.length === 0) {
    return { steps: [createPlanStep(goalActionName)], totalCost: goalAction.cost ?? 1, alternativeCount: 1 };
  }
